...
# Create new tracks
triggers = MidiTrack()
triggers.append(MetaMessage('track_name', name='TRIGGERS'))
        ...
        for msg in track:
            ...
            if msg.type in ['note_on', 'note_off']:
                ...
                if msg.note in [96, 97, 98]:
                    modified_msg = Message(type=msg.type, note=msg.note - 72, velocity=msg.velocity, time=cumulative_time)
                    triggers.append(modified_msg)
                    cumulative_time = 0  # Reset time after each note is added
                elif msg.note == 100:
                    modified_msg = Message(type=msg.type, note=msg.note - 74, velocity=msg.velocity, time=cumulative_time)
                    triggers.append(modified_msg)
                    cumulative_time = 0  # Reset time after each note is added
...
midi_file.tracks.extend([band_bass, triggers, guitar_fretmap])
