    ...
    if track.name == 'PART KEYS':
        ...
        for msg in track:
            ...
            # Verify if the event is a track name
                if msg.type != 'track_name':
                # Copy text events to the new track
                    temp_coop.append(msg.copy(time=cumulative_time))
                    cumulative_time = 0
                            
                # Remove 'PART GUITAR COOP' track
...
# Rename 'PART KEYS' to 'BAND SINGER' and remove notes from it
for track in midi_file.tracks:
...
midi_file.tracks.extend([temp_guitar, band_drums_track, temp_rhythm, temp_coop, triggers_track])
